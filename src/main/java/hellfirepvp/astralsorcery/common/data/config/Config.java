...
public class Config {
    ...
    private static void loadData() {
        giveJournalFirst = latestConfig.getBoolean("giveJournalAtFirstJoin", "general", true,
                "If set to 'true', the player will receive an AstralSorcery Journal when they join the server for the first time.");
        doesMobSpawnDenyDenyEverything = latestConfig.getBoolean("doesMobSpawnDenyAllTypes", "general", false,
        ...
        swordSharpMultiplier = latestConfig.getFloat("swordSharpenedMultiplier", "general", 0.1F, 0.0F, 10000.0F,
                "Defines how much the 'sharpened' modifier increases the damage of the sword if applied. Config value is in percent.");
        rockCrystalOreSilkTouchHarvestable = latestConfig.getBoolean("isRockCrystalOreSilkHarvestable", "general",
                rockCrystalOreSilkTouchHarvestable,
                "If this is set to true, Rock-Crystal-Ore may be silk-touch harvested by a player.");
        String[] dimWhitelist = latestConfig.getStringList("skySupportedDimensions", "general", new String[] { "0" },
                "Whitelist of dimension ID's that will have special sky rendering");
        String[] weakSkyRenders = latestConfig.getStringList("weakSkyRenders", "general", new String[] {},
        ...
        String[] oreModidBlacklist = latestConfig.getStringList("oreGenBlacklist", "general",
        ...
        dayLength = latestConfig.getInt("dayLength", "general", dayLength, 100, Integer.MAX_VALUE,
        ...
        ambientFlareChance = latestConfig.getInt("EntityFlare.ambientspawn", "entities", ambientFlareChance, 0, 200_000,
                "Defines how common ***ambient*** flares are. the lower the more common. 0 = ambient ones don't appear/disabled.");
        flareKillsBats = latestConfig.getBoolean("EntityFlare.killbats", "entities", true,
                "If this is set to true, occasionally, a spawned flare will (attempt to) kill bats close to it.");

        lightProximityAltarRecipe = latestConfig.getBoolean("LightProximity-Altar", "recipes",
        ...
        lightProximityResonatingWandRecipe = latestConfig.getBoolean("LightProximity-ResonatingWand", "recipes",
        ...
        illuminationWandUseCost = latestConfig.getFloat("wandCost_illumination", "tools", 0.5F, 0.0F, 1.0F,
                "Sets the quick-charge cost for one usage of the illumination wand");
        architectWandUseCost = latestConfig.getFloat("wandCost_architect", "tools", 0.03F, 0.0F, 1.0F,
                "Sets the quick-charge cost for one usage of the architect wand");
        exchangeWandUseCost = latestConfig.getFloat("wandCost_exchange", "tools", 0.002F, 0.0F, 1.0F,
                "Sets the quick-charge cost for one usage of the exchange wand");
        grappleWandUseCost = latestConfig.getFloat("wandCost_grapple", "tools", grappleWandUseCost, 0.0F, 1.0F,
                "Sets the quick-charge cost for one usage of the grapple wand");

        exchangeWandMaxHardness = latestConfig.getFloat("exchange_wand_max_hardness", "tools", -1, -1, 50000,
        ...
        capeChaosResistance = latestConfig.getFloat("cape_chaosresistance", "tools", capeChaosResistance, 0.0F, 1.0F,
                "Sets the amount of damage reduction a player gets when being hit by a DE chaos-damage-related damagetype.");

        shouldChargedToolsRevert = latestConfig.getBoolean("chargedCrystalToolsRevert", "tools",
                shouldChargedToolsRevert,
                "If this is set to true, charged crystals tools can revert back to their inert state.");
        revertStart = latestConfig.getInt("chargedCrystalToolsRevertStart", "tools", revertStart, 0,
                Integer.MAX_VALUE - 1,
                "Defines the minimum uses a user at least gets before it's trying to revert to an inert crystal tool.");
        revertChance = latestConfig.getInt("chargedCrystalToolsRevertChance", "tools", revertChance, 1,
                Integer.MAX_VALUE,
        ...
        evorsioEffectChance = latestConfig.getFloat("evorsioAttunedWandEffectChance", "tools", evorsioEffectChance, 0F,
                1F,
                "Defines the chance per mined block that the effect for holding an evorsio attuned resonating wand will fire.");
        discidiaStackCap = latestConfig.getInt("discidiaDamageStackCap", "tools", discidiaStackCap, 1, 200,
                "Defines the amount of stacks you have to get against the same mob until you reach 100% of the damage multiplier.");
        discidiaStackMultiplier = latestConfig.getFloat("discidiaDamageStackMultipler", "tools",
                discidiaStackMultiplier, 0F, 200F,
        ...
        grindstoneAddDustRecipes = latestConfig.getBoolean("grindstoneAddOreToDustRecipes", "crafting", true,
                "Set this to false to prevent the lookup and registration of oreblock -> ore dust recipes on the grindstone.");

        craftingLiqCrystalGrowth = latestConfig.getBoolean("liquidStarlightCrystalGrowth", "crafting", true,
                "Set this to false to disable Rock/Celestial Crystal growing in liquid starlight.");
        craftingLiqCelestialCrystalForm = latestConfig.getBoolean("liquidStarlightCelestialCrystalCluster", "crafting",
                true, "Set this to false to disable crystal + stardust -> Celestial Crystal cluster forming");
        craftingLiqCrystalToolGrowth = latestConfig.getBoolean("liquidStarlightCrystalToolGrowth", "crafting",
                craftingLiqCrystalToolGrowth, "Set this to false to disable Crystal Tool growth in liquid starlight");
        canCrystalGrowthYieldDuplicates = latestConfig.getBoolean("canCrystalGrowthYieldDuplicates", "crafting",
        ...
        liquidStarlightAquamarine = latestConfig.getBoolean("liquidStarlightAquamarine", "crafting",
        ...
        liquidStarlightSand = latestConfig.getBoolean("liquidStarlightSand", "crafting", liquidStarlightSand,
                "Set this to false to disable that liquid starlight + lava produces sand.");
        liquidStarlightIce = latestConfig.getBoolean("liquidStarlightIce", "crafting", liquidStarlightIce,
                "Set this to false to disable that liquid starlight + water produces ice.");
        liquidStarlightInfusedWood = latestConfig.getBoolean("liquidStarlightInfusedWood", "crafting",
        ...
        performNetworkIntegrityCheck = latestConfig.getBoolean("performNetworkIntegrityCheck", "lightnetwork", false,
        ...
        maxEffectRenderDistance = latestConfig.getInt("maxEffectRenderDistance", "rendering", 64, 1, 512,
        ...
        clientPreloadTextures = latestConfig.getBoolean("preloadTextures", "rendering", true,
        ...
        particleAmount = latestConfig.getInt("particleAmount", "rendering", 2, 0, 2,
        ...
        disableFestiveMapper = latestConfig.getBoolean("disableFestiveBlockTextures", "rendering", false,
                "Set to true to disable the festive textures/block models.");

        marbleAmount = latestConfig.getInt("generateMarbleAmount", "worldgen", 4, 0, 32,
                "Defines how many marble veins are generated per chunk. 0 = disabled");
        marbleVeinSize = latestConfig.getInt("generateMarbleVeinSize", "worldgen", 20, 1, 32,
                "Defines how big generated marble veins are.");
        aquamarineAmount = latestConfig.getInt("generateAquamarineAmount", "worldgen", 64, 0, 2048,
                "Defines how many aquamarine ores it'll attempt to generate in per chunk. 0 = disabled");
        constellationPaperRarity = latestConfig.getInt("constellationPaperRarity", "worldgen", 10, 1, 128,
                "Defines the rarity of the constellation paper item in loot chests.");
        constellationPaperQuality = latestConfig.getInt("constellationPaperQuality", "worldgen", 2, 1, 128,
                "Defines the quality of the constellation paper item in loot chests.");
        respectIdealDistances = latestConfig.getBoolean("respectIdealStructureDistances", "worldgen",
        ...
        String[] dimGenWhitelist = latestConfig.getStringList("worldGenWhitelist", "worldgen", new String[] { "0" },
                "the Astral Sorcery-specific worldgen will only run in Dimension ID's listed here.");
        enableFlatGen = latestConfig.getBoolean("enableFlatGen", "worldgen", false,
        ...
        enableRetroGen = latestConfig.getBoolean("enableRetroGen", "retrogen", false,
        ...
        enablePatreonEffects = latestConfig.getBoolean("enablePatreonEffects", "patreon", enablePatreonEffects,
